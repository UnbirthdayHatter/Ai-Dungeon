...
interface EmeraldThreeDiceProps {
  results: number[];
  diceType: number;
  total?: number;
  label?: string;
...
export function EmeraldThreeDice({
  results,
  diceType,
  total,
  label,
  ...
  useEffect(() => {
    ...
    scene.add(new THREE.AmbientLight(0xd4ffe8, 0.56));

    const keyLight = new THREE.DirectionalLight(0x8fffe5, 1.48);
    keyLight.position.set(4.2, 10.8, 3.6);
    keyLight.castShadow = true;
    keyLight.shadow.mapSize.set(1024, 1024);
    scene.add(keyLight);

    const rimLight = new THREE.PointLight(0x34d399, 7.2, 20, 2);
    rimLight.position.set(-2.2, 3.8, 2.0);
    scene.add(rimLight);
    ...
    const dice = effectiveResults.map((_, index) => {
      ...
      const body = new CANNON.Body({
        ...
        material: new CANNON.Material('emerald'),
        allowSleep: true,
    ...
    const animate = () => {
      ...
      dice.forEach((die, index) => {
        die.materials.forEach((material, faceIndex) => {
          ...
          material.uniforms.uSeed.value = 0.33 + index * 0.21 + faceIndex * 0.11;
        });
      ...
      rimLight.intensity = 6.6 + Math.sin(elapsed * 1.8) * 0.5;
      composer.render();
